...
with open('extension_pooling_layers/mnist_full_np.pkl', 'wb') as file:
...
for model, model_name in zip(models, model_names.keys()):
    ...
    for norm, epsilons in attack_params:
        ...
        for epsilon in epsilons:
            if norm == 2:
                attack = ProjectedGradientDescentTensorFlowV2(estimator=classifier, eps=epsilon, eps_step=epsilon/5, max_iter=100, batch_size=100, norm=norm) #changed to 100
            else:
...
with open('extension_pooling_layers/mnist_accuracy_data.pkl', 'wb') as file:
    pickle.dump(accuracy_data, file)

# Save the model names
with open('extension_pooling_layers/mnist_model_names.pkl', 'wb') as file:
