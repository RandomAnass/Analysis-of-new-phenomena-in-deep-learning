...
# nb_epochs=20
def create_art_classifier(model_creator, x_train, y_train, x_test, y_test, batch_size=200, nb_epochs=10, **kwargs):
    ...
    for add_dense in add_dense_list:
        for max_pooling in max_pooling_list:
            for kernel_size in kernel_size_list:
                model_name = f"simple_Conv_{n_layers}_{max_pooling}_{kernel_size}"
...
attack_params = [[np.inf, [0.05, 0.15,0.25]],[2, [0.5,  1.5, 3]]]
...
with open('mnist_accuracy_data.pkl', 'wb') as file:
    pickle.dump(accuracy_data, file)

# Save the model names
with open('mnist_model_names.pkl', 'wb') as file:
